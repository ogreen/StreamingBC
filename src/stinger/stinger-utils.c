                        ...
                        const int64_t type)
{
  ...
  struct stinger_eb *start = ebpool + S->LVA[srcvtx].edges;
  while (start != ebpool && start->etype != type) {
    start = ebpool + start->next;
  }
    ...
    while (cur_eb != ebpool && cur_eb->etype == type) {
      for (int64_t i = 1; i < STINGER_EDGEBLOCKSIZE; i += 2) {
        if (i < STINGER_EDGEBLOCKSIZE - 1) {
          ...
          if (cur_eb->next && ebpool[cur_eb->next].etype == type
              && cur_eb->edges[i].neighbor >
              ebpool[cur_eb->next].edges[0].neighbor) {
            struct stinger_edge tmp = ebpool[cur_eb->next].edges[0];
            ebpool[cur_eb->next].edges[0] = cur_eb->edges[i];
            cur_eb->edges[i] = tmp;
            sorted = 0;
          }
        }
      }
      cur_eb = ebpool + cur_eb->next;
    }
