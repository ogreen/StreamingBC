...
stinger_iterator_next_block_by_type(stinger_iterator_t * iter) {
  ...
  while(1) {
    ...
    if(iter->i.edge_block_index < iter->i.s->ETA[curtype].high) {
      iter->i.cur_eb = ebpool + iter->i.s->ETA[curtype].blocks[iter->i.edge_block_index];
...
stinger_iterator_next_block_by_vtx(stinger_iterator_t * iter) {
  ...
  if(iter->i.cur_eb && iter->i.cur_eb->next) {
    iter->i.cur_eb = ebpool + iter->i.cur_eb->next;
  } else {
    while(1) {
      iter->i.vtx_index++;
      if(iter->i.vtx_index < iter->i.vtx_filter_count) {
	iter->i.cur_eb = ebpool + iter->i.s->LVA[iter->i.vtx_filter[iter->i.vtx_index]].edges;
	if(iter->i.cur_eb) {
	  stinger_iterator_get_metadata(iter);
	  return 0;
	}
...
int
stinger_iterator_check_vtype(stinger_iterator_t * iter) {
  int64_t src_type = stinger_vtype(iter->i.s, iter->i.cur_eb->vertexID);
  int64_t dst_type = stinger_vtype(iter->i.s, iter->i.cur_eb->edges[iter->i.cur_edge].neighbor);
